...
function AddFuncao() {
	...
	funcao.fu_atividade = $fu_atividade;
	...
	for (var i = 0; i < arrFuncao.length; i++) {
		...
		if (arrFuncao[i].experiencia_ctps != 'Sim') {
			...
			$html_funcao += '<td>' + arrFuncao[i].fu_atividade + '</td>';
			...
			$html_funcao += '<td><b>' + arrFuncao[i].fu_atividade + '</td></b>';
...
function FunAttCurriculo(cadastro) {
	...
	$data += 'ocupacao=' + cadastro.ocupacao + "&";
...
function DelCurriculo(num_cadastro, cu_status, cu_motivo) {

	$data = '';
	$data += 'num_cadastro=' + num_cadastro + '&';
	$data += 'cu_status=' + cu_status + '&';
	$data += 'cu_motivo=' + cu_motivo;
		...
		success: function (data) {
			...
			if (data == 0) {
                ...
                switch(cu_status){
                    ...
                    case 'A': window.alert("Curriculo ativado sucesso!");
                    case 'I': window.alert("Curriculo inativado sucesso!");
...
function relOcupacoes() {

    $.ajax({

        url: '/agtbr/sys/functions/RelOcupacoes.php',
        dataType: 'json',
        success: function (data) {

            /*function ShowCRM(item) {
                $('#funcao').html(item.value);
                GetProcedimentos(item.value);
                GetAgenda(data[0].id);
            }*/
